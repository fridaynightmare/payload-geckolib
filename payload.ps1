...
$ports = @(
    ...
    81,88,514,515,548,631,1080, 
    1433,1434,3306,5432,6379,1521,27017, 
    3389,5555,5900,5901,5985,5986, 
    8000,8080,8081,8083,8443,8888,9000,9090,9100,
    161,162,2049,10000, 
    5555,6667,8123,32400, 
    49152,49153,49154 
)
    ...
    if ($localIP) {
        ...
        $sb = { param($ip, $p)
            $foundPorts = foreach($port in $p){
                ...
                if($t.BeginConnect($ip,$port,$null,$null).AsyncWaitHandle.WaitOne(150)){ if($t.Connected){$port} }
                $t.Close(); $t.Dispose()
            }
            if($foundPorts){
                $mac = (arp -a $ip | Select-String -Pattern "([0-9a-fA-F]{2}-){5}[0-9a-fA-F]{2}").Matches.Value
                if(!$mac){ $mac = "Unknown" }
                "IP: $ip | MAC: $mac | Ports: $($foundPorts -join ',')"
            }
        }
